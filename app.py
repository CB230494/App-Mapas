...
import io, json, zipfile, tempfile, uuid, re, datetime as dt
...
def ws_connect():
    ...
    import gspread
    sa = _get_sa_info_from_secrets()
    if not sa:
        raise RuntimeError("Falta [google_service_account] o [gcp_service_account] en secrets.toml")
    ssid = st.secrets.get("SHEETS_SPREADSHEET_ID","").strip()
    wsname = st.secrets.get("SHEETS_WORKSHEET_NAME","Hoja 1").strip()
    if not ssid: raise RuntimeError("Falta SHEETS_SPREADSHEET_ID en secrets.toml")
    ...
    try:
        ...
        ws = sh.add_worksheet(wsname, rows="100", cols="20")
    # Encabezado si vacío
...
for lname, meta in list(st.session_state.layers.items()):
    with st.sidebar.expander(lname, expanded=False):
        ...
        if st.button("Eliminar capa", key=f"del_layer_{lname}"):
            ...
            if _sheets_ok:
                try: save_layers_to_ws(ws0)
                except Exception as e: st.toast(f"No se pudo guardar en Sheets: {e}", icon="🟠")
            st.rerun()
...
with st.sidebar.expander("➕ Agregar capa"):
    ...
    if st.button("Crear capa", key="btn_new_layer"):
        if new_name and new_name not in st.session_state.layers:
            st.session_state.layers[new_name] = {"color": _clean_hex(new_color), "visible": True, "features": []}
            if _sheets_ok:
                try: save_layers_to_ws(ws0)
                except Exception as e: st.toast(f"No se pudo guardar en Sheets: {e}", icon="🟠")
            st.rerun()
...
with tab_mapa:
    ...
    cantones = ["(todos)"] + (CR_CATALOG.get(provincia_sel, []) if provincia_sel != "(todas)" else sorted({c for v in CR_CATALOG.values() for c in v}))
    canton_sel = st.selectbox("Cantón", cantones, index=0, key="canton_map")

    st.subheader("📝 Ficha del caso para el próximo punto")
    c1, c2, c3, c4 = st.columns([1,1,1,1])
    with c1: layer_active = st.selectbox("Capa activa", list(st.session_state.layers.keys()), key="layer_active")
    ...
    desc = st.text_area("Descripción (máx. 240)", "Rehabilitación de iluminación y mobiliario; patrullajes.", key="desc_map")[:240]
    ...
    cant_guardar = (canton_sel if canton_sel != "(todos)" else (cant_list[0] if cant_list else ""))
    with d2a: st.text_input("Provincia (auto)", value=prov_guardar, key="prov_auto", disabled=True)
    with d2b: st.text_input("Cantón (auto)", value=cant_guardar, key="cant_auto", disabled=True)
    ...
    def pass_filter(p: Dict[str, Any]) -> bool:
        if provincia_sel != "(todas)" and p.get("provincia") != provincia_sel: return False
        if canton_sel != "(todos)" and p.get("canton") != canton_sel: return False
        return True

    heat_points = []
    for lname, meta in st.session_state.layers.items():
        if not meta.get("visible", True): continue
        ...
        for f in meta.get("features", []):
            ...
            folium.Marker([lat, lon], icon=icon, tooltip=p.get('titulo','(Caso)'),
    ...
    if show_heat and heat_points:
        HeatMap(heat_points, radius=25, blur=25, min_opacity=0.3, name="Heatmap").add_to(m)
    folium.LayerControl(collapsed=False).add_to(m)

    state = st_folium(m, height=640, key="mapa_main")
    click = state.get("last_clicked") if state else None
    if click: st.session_state.last_click = (float(click["lat"]), float(click["lng"]))

    cA, cB, cC = st.columns(3)
    with cA: st.info("Haz clic en el mapa para elegir ubicación." if not st.session_state.last_click else f"Último clic: {st.session_state.last_click}")
    with cB:
        add_ok = st.button("➕ Confirmar agregar punto aquí", key="btn_add", disabled=(st.session_state.last_click is None))
    with cC:
        if st.button("🧹 Limpiar selección de clic", key="btn_clear", disabled=(st.session_state.last_click is None)):
            st.session_state.last_click = None; st.rerun()

    # Agregar y guardar a Sheets
    if add_ok and st.session_state.last_click:
        lat, lon = st.session_state.last_click
        props = {
            "id": _new_id(), "layer": layer_active, "color": _clean_hex(st.session_state.layers[layer_active]["color"]),
            "titulo": titulo.strip(), "desc": desc.strip(), "fecha": str(fecha),
            "provincia": prov_guardar, "canton": cant_guardar, "responsable": responsable,
            "impacto": impacto.strip(), "enlace": enlace.strip()
        }
        st.session_state.layers[layer_active]["features"].append(
            {"type":"Feature","properties":props,"geometry":{"type":"Point","coordinates":[float(lon), float(lat)]}}
        )
    ...
    st.divider(); st.subheader("📋 Gestión por capas (eliminar / **editar / mover**)")
    ...
    for i, lname in enumerate(list(st.session_state.layers.keys())):
        with subtabs[i]:
            ...
            if not feats:
                st.info("Sin casos aún."); continue
            ...
            with c1:
                idx_del = st.number_input("Eliminar (índice)", 0, len(feats)-1, 0, 1, key=f"idx_del_{lname}")
            ...
            with c2:
                ...
                ied = st.number_input("Índice", 0, len(feats)-1, 0, 1, key=f"idx_edit_{lname}")
                f = feats[int(ied)]; p = f["properties"]
                with st.form(f"form_edit_{lname}"):
                    t = st.text_input("Título", p.get("titulo",""), key=f"t_{lname}")
                    fe = st.date_input("Fecha", value=pd.to_datetime(p.get("fecha", dt.date.today())).date(), key=f"fe_{lname}")
                    resp = st.selectbox("Responsable", ["GL","FP","Mixta"],
                                        index=["GL","FP","Mixta"].index(p.get("responsable","GL")), key=f"resp_{lname}")
                    provs_edit = list(CR_CATALOG.keys())
                    prov_idx = provs_edit.index(p.get("provincia","San José")) if p.get("provincia","San José") in provs_edit else 0
                    prov = st.selectbox("Provincia", provs_edit, index=prov_idx, key=f"prov_{lname}")
                    clist = CR_CATALOG.get(prov, []); cval = p.get("canton", clist[0] if clist else "")
                ...
                if ok:
                    p.update({"titulo": t.strip(),"fecha": str(fe),"responsable": resp,
                              "provincia": prov,"canton": cant,"impacto": imp.strip(),
                              "enlace": enl.strip(),"desc": des.strip()})
                    if _sheets_ok:
                        try: save_layers_to_ws(ws0)
                        except Exception as e: st.toast(f"No se pudo guardar en Sheets: {e}", icon="🟠")
            ...
            with c3:
                ...
                imv = st.number_input("Índice", 0, len(feats)-1, 0, 1, key=f"idx_move_{lname}")
                if st.button("🔀 Activar mover por clic", key=f"btn_move_{lname}"):
                    st.session_state.move_target = (lname, int(imv)); st.info("Haz clic en el mapa para mover.")
...
with tab_dashboard:
    ...
    if df.empty:
        ...
        cant_f = st.selectbox("Cantón", ["(todos)"] + (CR_CATALOG.get(prov_f, []) if prov_f!="(todas)" else sorted({c for v in CR_CATALOG.values() for c in v})), 0, key="cant_dash")
        fdf = df[df["Capa"].isin(capa_f)].copy()
        if prov_f!="(todas)": fdf = fdf[fdf["Provincia"]==prov_f]
        if cant_f!="(todos)": fdf = fdf[fdf["Cantón"]==cant_f]
        fdf["Fecha"] = pd.to_datetime(fdf["Fecha"], errors="coerce")
        fdf["Año-Mes"] = fdf["Fecha"].dt.to_period("M").astype(str)
        c1, c2 = st.columns(2)
        with c1: st.markdown("**Casos por capa**"); st.bar_chart(fdf.groupby("Capa")["Título"].count().sort_values(ascending=False))
        with c2: st.markdown("**GL/FP/Mixta**"); st.bar_chart(fdf.groupby("Resp")["Título"].count())
        c3, c4 = st.columns(2)
        with c3: st.markdown("**Por provincia**"); st.bar_chart(fdf.groupby("Provincia")["Título"].count().sort_values(ascending=False))
        with c4: st.markdown("**Serie mensual**"); st.line_chart(fdf.groupby("Año-Mes")["Título"].count())
        st.divider(); st.markdown("**Tabla (filtrada)**"); st.dataframe(fdf, use_container_width=True)

# ========= 📤 EXPORT =========
with tab_export:
    st.subheader("Exportar todo (todas las capas)")
    fc = all_features_fc(); gdf = gdf_from_fc(fc)
    def _geojson_bytes(): return json.dumps(fc, ensure_ascii=False).encode("utf-8")
    def _csv_bytes():
        if gdf.empty: return b""
        df = pd.DataFrame(gdf.drop(columns="geometry")); df["lat"]=gdf.geometry.y; df["lon"]=gdf.geometry.x
        return df.to_csv(index=False).encode("utf-8")
    def _shp_zip():
        if gdf.empty: return b""
        with tempfile.TemporaryDirectory() as td:
            path = Path(td)/"casos_exito.shp"; gdf.to_file(path, driver="ESRI Shapefile", encoding="utf-8")
            bio = io.BytesIO()
            with zipfile.ZipFile(bio, "w", zipfile.ZIP_DEFLATED) as zf:
                for p in Path(td).glob("casos_exito.*"): zf.write(p, arcname=p.name)
            bio.seek(0); return bio.getvalue()
    c1, c2, c3 = st.columns(3)
    with c1: st.download_button("⬇️ GeoJSON", data=_geojson_bytes(), file_name=f"{st.session_state.project_name}.geojson", mime="application/geo+json", disabled=(len(fc['features'])==0))
    with c2: st.download_button("⬇️ Shapefile (ZIP)", data=_shp_zip(), file_name=f"{st.session_state.project_name}.zip", mime="application/zip", disabled=(len(fc['features'])==0))
    with c3: st.download_button("⬇️ CSV", data=_csv_bytes(), file_name=f"{st.session_state.project_name}.csv", mime="text/csv", disabled=(len(fc['features'])==0))
...
with tab_sheets:
    ...
    if _sheets_ok:
        try:
            ws = ws_connect()
            st.success(f"✅ Conectado • Hoja: {st.secrets.get('SHEETS_WORKSHEET_NAME','Hoja 1')}")
            st.info(f"📄 Filas actuales: {len(ws.get_all_values())}")
            c1, c2 = st.columns(2)
            with c1:
                if st.button("⬇️ Forzar cargar desde Sheets"):
                    if load_layers_from_ws(ws): st.success("Datos cargados."); st.rerun()
                    else: st.info("La hoja está vacía (sólo encabezado).")
            with c2:
                if st.button("⬆️ Forzar subir (reemplazar hoja)"):
                    save_layers_to_ws(ws); st.success("Datos subidos.")
        except Exception as e:
